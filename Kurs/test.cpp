...
int main()
{
	...
	for ( int i = 0; i < 100; ++i )
	{
		...
		if ( NameIndex < 202 )
			printf("%sa\t%-15s\t%d-%d-%d\tг.Москва,ул.%-15s,д.%d,кв.%d\n", Surnames[SurnameIndex], Names[NameIndex], 1960 + rand()%37, 1 + rand()%12, 1 + rand()%29, Streets[StreetIndex], 1 + rand()%MAXHOUSENUMBER, 1 + rand()%MAXFLATNUMBER );
		else
			printf("%s\t%-15s\t%d-%d-%d\tг.Москва,ул.%15s,д.%d,кв.%d\n", Surnames[SurnameIndex], Names[NameIndex], 1960 + rand()%37, 1 + rand()%12, 1 + rand()%29, Streets[StreetIndex], 1 + rand()%MAXHOUSENUMBER, 1 + rand()%MAXFLATNUMBER );
	}
}	
